...
?>
<?php
    $sql = "select max(id) as maximo from peliculas";
    $salida = sqlsrv_query($conn, $sql);
    $mostrar = sqlsrv_fetch_array($salida);
    /*?> <h> <?php echo Datos ingresados correctamente; ?> </h> <?php
    */
    $sql = "insert into peliculas values((".$mostrar['maximo']."+1),'".$_POST["nombre"]."')";
